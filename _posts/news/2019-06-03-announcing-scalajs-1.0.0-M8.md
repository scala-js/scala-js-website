---
layout: post
title: Announcing Scala.js 1.0.0-M8
category: news
tags: [releases]
permalink: /news/2019/06/03/announcing-scalajs-1.0.0-M8/
---
...
* The `JSEnv` API has been refactored to use `java.nio.file.Path`s instead of custom virtual files
* The linker API is now fully asynchronous; clean separation into different compatibility layers is pending ([#3656](https://github.com/scala-js/scala-js/issues/3656))
...
Before upgrading to 1.0.0-M8, **we strongly recommend that you upgrade to Scala.js 0.6.28 or later**, and address all deprecation warnings.
Since Scala.js 1.0.0-M8 removes support for all the deprecated features in 0.6.x, it is easier to see the deprecation messages guiding you to the proper replacements.
...
The old `crossProject` is deprecated in 0.6.28, but it is easy to overlook deprecations in the build itself.
...
* Add `addSbtPlugin("org.scala-js" % "sbt-jsdependencies" % "1.0.0-M8")` in `project/plugins.sbt`
...
* Add `libraryDependencies += "org.scala-js" %% "scalajs-env-jsdom-nodejs" % "1.0.0-M8"` in `project/plugins.sbt`

If you use the PhantomJS environment:

* Add `addSbtPlugin("org.scala-js" % "sbt-scalajs-env-phantomjs" % "1.0.0-M8")` in `project/plugins.sbt`
...
This should get your build up to speed to Scala.js 1.0.0-M8.
...
* `org.scalajs.core.tools.io`      -> gone (replaced by standard `java.nio.file.Path`-based APIs, and some abstractions in `org.scalajs.linker.irio`)
...
There are very few enhancements in Scala.js 1.0.0-M8.
